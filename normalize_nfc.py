...
def normalize_filenames_to_nfc(target_dir='.', verbose=False, replace_spaces=None):
    # 먼저 모든 파일/폴더 경로를 수집
    all_items = []
    items_to_rename = []
    ...
    for root, dirs, files in os.walk(target_dir):
        ...
        for name in files:
            ...
            if replace_spaces is not None:
                nfc_name = nfc_name.replace(' ', replace_spaces)
    ...
    # 공백 변경 옵션 상태 표시
    if replace_spaces is not None:
        print(f"Space replacement: ' ' -> '{replace_spaces}'")
    ...
    if not items_to_rename:
        if replace_spaces is not None:
            print("모든 파일과 폴더가 이미 NFC 형태이고 공백 변경이 필요하지 않습니다.")
        else:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='예시: python3 normalize_nfc.py /Users/yourname/Documents --replace-spaces'
    )
    ...
    parser.add_argument(
        ...
        const='_',
        default=None,
        help='공백을 지정한 문자로 변경 (기본값: _). 예: --replace-spaces 또는 --replace-spaces -'
    )
    ...
    if args.verbose:
        ...
        if args.replace_spaces is not None:
            print(f"Spaces will be replaced with: '{args.replace_spaces}'")

    try:
        normalize_filenames_to_nfc(
            target_path,
            args.verbose,
            args.replace_spaces
        )
